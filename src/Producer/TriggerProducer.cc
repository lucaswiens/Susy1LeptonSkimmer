...
void TriggerProducer::Produce(CutFlow &cutflow, Susy1LeptonProduct *product) {
	...
	HLT_LepOr = false;
	HLT_MetOr = false;
	TriggerEfficiency = -999;


	if (era ==2016) {
		HLT_EleOr = *hlt_Ele105_CaloIdVT_GsfTrkIdT->Get() || *hlt_Ele115_CaloIdVT_GsfTrkIdT->Get() || *hlt_Ele50_CaloIdVT_GsfTrkIdT_PFJet165->Get() || *hlt_Ele27_WPTight_Gsf->Get() || *hlt_Ele15_IsoVVVL_PFHT350->Get() || *hlt_Ele15_IsoVVVL_PFHT400->Get();
		HLT_MuOr = *hlt_Mu50->Get() || *hlt_IsoMu24->Get() || *hlt_IsoTkMu24->Get() || *hlt_Mu15_IsoVVVL_PFHT350->Get() || *hlt_Mu15_IsoVVVL_PFHT400->Get();
		...
		if (abs(product->leptonPdgId) == 11){
			TriggerEfficiency = 1;
		} else if (abs(product->leptonPdgId) == 13) {
			TriggerEfficiency = 1;
		} else {
			TriggerEfficiency = 1;
		}
